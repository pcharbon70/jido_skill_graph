defmodule Jido.SkillsetTest do
  ...
  test "build/1 returns a snapshot struct" do
    assert {:ok, %Snapshot{} = snapshot} =
             Jido.Skillset.build(root: fixture_path("basic"), graph_id: "knowledge-work")
  ...
  test "build/1 validates unresolved link policy" do
    assert {:error, {:invalid_unresolved_link_policy, :drop}} =
             Jido.Skillset.build(root: fixture_path("basic"), unresolved_link_policy: :drop)
  end
