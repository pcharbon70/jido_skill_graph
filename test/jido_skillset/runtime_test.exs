defmodule Jido.Skillset.RuntimeTest do
  use ExUnit.Case, async: true

  alias Jido.Skillset.{Builder, Loader, Snapshot, Store}
  ...
  test "store publishes snapshots with ETS node, edge, and search indexes" do
    ...
    assert %Jido.Skillset.Node{id: "alpha"} = Snapshot.get_node(committed, "alpha")
  ...
  test "facade supervisor wiring connects configured store and loader names" do
    ...
    assert :ok = Jido.Skillset.reload(loader_name)
    assert %Snapshot{} = Jido.Skillset.current_snapshot(store_name)
  end
