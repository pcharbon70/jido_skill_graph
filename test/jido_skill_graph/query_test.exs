defmodule JidoSkillGraph.QueryTest do
  ...
  test "list_graphs/1 is empty when no snapshot is loaded" do
    ...
    assert JidoSkillGraph.list_graphs(store: store_name) == []
    assert {:error, :graph_not_loaded} = JidoSkillGraph.topology("basic", store: store_name)
  end

  test "topology/2 returns summary and optional topology details" do
    {store_name, _loader_name} = load_graph("cycle", "cycle")

    assert {:ok, topology} = JidoSkillGraph.topology("cycle", store: store_name)
  ...
  test "node metadata and body reads are available through facade" do
    ...
    assert {:ok, nodes} = JidoSkillGraph.list_nodes("basic", store: store_name)
    assert nodes |> Enum.map(& &1.id) |> Enum.sort() == ["alpha", "beta"]

    assert {:ok, [tagged]} = JidoSkillGraph.list_nodes("basic", store: store_name, tags: ["core"])
    assert tagged.id == "alpha"

    assert {:ok, alpha_meta} = JidoSkillGraph.get_node_meta("basic", "alpha", store: store_name)
    assert alpha_meta.title == "Alpha"

    assert {:ok, body} = JidoSkillGraph.read_node_body("basic", "alpha", store: store_name)
    assert String.contains?(body, "Alpha references")

    assert {:ok, payload} =
             JidoSkillGraph.read_node_body("basic", "alpha",
               store: store_name,
    ...
    assert {:error, {:unknown_node, "missing"}} =
             JidoSkillGraph.get_node_meta("basic", "missing", store: store_name)
  end

  test "link traversal APIs support rel filtering and neighbor hops" do
    {store_name, _loader_name} = load_graph("basic", "basic")

    assert {:ok, out_links} = JidoSkillGraph.out_links("basic", "alpha", store: store_name)
    assert length(out_links) == 2

    assert {:ok, prereq_links} =
             JidoSkillGraph.out_links("basic", "alpha", store: store_name, rel: :prereq)

    assert Enum.all?(prereq_links, &(&1.rel == :prereq))

    assert {:ok, in_links} = JidoSkillGraph.in_links("basic", "beta", store: store_name)
    assert length(in_links) == 2

    assert {:ok, ["beta"]} =
             JidoSkillGraph.neighbors("basic", "alpha",
               store: store_name,
               direction: :out,
               hops: 2
             )
    ...
    assert {:ok, ["alpha"]} =
             JidoSkillGraph.neighbors("basic", "beta", store: store_name, direction: :in, hops: 1)

    assert {:error, {:invalid_relation_filter, :depends_on}} =
             JidoSkillGraph.out_links("basic", "alpha", store: store_name, rel: :depends_on)
  end
