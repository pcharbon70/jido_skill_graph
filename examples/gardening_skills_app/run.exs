defmodule GardeningSkillsApp do
  ...
  def run do
    ...
    {:ok, search_results} = JidoSkillGraph.search(graph_id, "soil compost", store: @store_name, limit: 3)
    IO.inspect(Enum.map(search_results, &Map.take(&1, [:id, :score, :matches])))

    print_header("Query: read_node_body/3 with frontmatter")

    {:ok, payload} =
      JidoSkillGraph.read_node_body(graph_id, "compost",
        store: @store_name,
